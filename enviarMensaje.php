...
$libro = $sentencia->fetch(PDO::FETCH_OBJ);
    ...
    $data = [
        ...
        "message" =>  'Estimado(a) su reserva de libro se realizo correctamente "*'.strtoupper($libro->nombreLibro).'" "'.strtoupper($libro->genero).'" "'.strtoupper($libro->autor).'*" No se pierda *'.strtoupper($libro->promocion).'* valido solo *'.$libro->duracion.'*'
    ];
    $options = array(
        'http' => array(
            'method'  => 'POST',
